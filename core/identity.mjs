...
// 两类别名：
//   1) space/org 别名：仓转移后旧坐标（github__olduser2__repo2）→ 现位置（github__Asklear__repo2）。
...
//   2) 身份别名：producer-id（user1，文件名前缀/frontmatter）与 git author（username1，commit 作者）是两套标识。
//      resolveAuthorQuery 用 team.yaml 花名册把 user1/username1/git_names 收敛成同一 accept 集，查任一别名结果一致。
...
// author 查询归一：把用户输入的一个名字（user1 / username1 / git_names 任一）解析成一个 accept 集。
// 命中花名册 → accept = 该成员的全部别名（id + name + git_names）：查 user1 与查 username1 落到同一集合 → 结果一致。
