...
Create `feishu.yaml` (copy from `feishu.example.yaml`, **secret → gitignored**) with your Lark/Feishu custom-app credentials to mirror wiki doc bodies into the truth store. Leave it out and the doc layer stays quietly off. Restart to pick up config changes.
