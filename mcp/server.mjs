...
const INSTRUCTIONS = `团队大脑（team-brain）：全队 Claude Code/Codex 的 session 汇成一个 git 真相库（每条 session 一份【脱敏全文对话】），再叠 GitHub/GitLab/Gitea 代码现状 + 人写文档镜像（飞书 wiki / Notion / Google Docs）。
...
console.error("team-brain MCP (0.6) 已启动：grep + find + read + ls + sessions + stats + log + read_github（含 feishu/·notion/·google/ 文档镜像）→", cfg.server_url);
