...
const INSTRUCTIONS = `团队大脑（team-brain）：全队 Claude Code/Codex 的 session 汇成一个 git 真相库（每条 session 一份【脱敏全文对话】），再叠 GitHub/GitLab/Gitea 代码现状 + 飞书文档镜像。
...
server.registerTool(
  "stats",
  {
    ...
    inputSchema: {
      ...
      until: z.string().optional().describe("【工作时间】上限（含），如 '2026-06-07'"),
      space: z.string().optional().describe("收窄到某 space_key（别名/历史 key 也接受）"),
      author: z.string().optional().describe("收窄到某人（producer-id 或 git 名，tqt/taoqitian 等价）"),
  ...
  async ({ by, split, metric, since, until, space, author, tool, limit, offset }) => {
    const r = await api("/stats", { by, split, metric, since, until, space, author, tool, limit, offset });
...
console.error("team-brain MCP (0.6) 已启动：grep + find + read + ls + sessions + stats + log + read_github（含 feishu/ 文档镜像）→", cfg.server_url);
