...
function buildTruth() {
  ...
  putCodex(s, "github__Asklear__bossa", "main", "tqt", "rollout-1", "etl/node-first-on-spec022");
  // 已在正确分支目录 → 不应动
  putCodex(s, "github__Asklear__bossa", "feat-x", "tqt", "rollout-2", "feat-x");
  // 旧 rollout 无 git 块 → 分支不可恢复，保持原样
  const d = join(s, "github__Asklear__bossa", "sessions", "main");
  writeFileSync(join(d, "tqt-rollout-old.jsonl"),
    `${JSON.stringify({ type: "session_meta", payload: { cwd: "/w/repo" } })}\n${JSON.stringify({ type: "event_msg", payload: { type: "user_message", message: "hi" } })}\n`);
  writeFileSync(join(d, "tqt-rollout-old.md"), `---\nid: old\ntool: codex\nbranch: main\n---\n# x\n`);
  // CC session（每行记分支）在 main → 不是 codex，跳过不动
  writeFileSync(join(d, "hank-cc1.jsonl"), `${JSON.stringify({ type: "user", cwd: "/w", gitBranch: "main", message: { content: "hi" } })}\n`);
  writeFileSync(join(d, "hank-cc1.md"), `---\nid: cc1\ntool: claude-code\nbranch: main\n---\n# x\n`);
  return truth;
}

test("fix-codex-branch: 按原文真实分支重分桶 + 重写卡片", () => {
  const truth = buildTruth();
  const s = join(truth, "spaces", "github__Asklear__bossa", "sessions");
  ...
  assert.ok(existsSync(join(dst, "tqt-rollout-1.jsonl")), "原文应移到正确分支目录");
  assert.ok(existsSync(join(dst, "tqt-rollout-1.md")));
  assert.ok(!existsSync(join(s, "main", "tqt-rollout-1.jsonl")), "源应被移走");
  // 卡片 branch 字段重写成真实分支（保留斜杠原值）
  const card = readFileSync(join(dst, "tqt-rollout-1.md"), "utf8");
  assert.match(card, /^branch: etl\/node-first-on-spec022$/m);

  // rollout-2 已在正确目录 → 不动
  assert.ok(existsSync(join(s, "feat-x", "tqt-rollout-2.jsonl")));
  // 旧无 git 块的、CC 的 → 留在 main
  assert.ok(existsSync(join(s, "main", "tqt-rollout-old.jsonl")), "无 git 块的旧 rollout 保持原样");
  assert.ok(existsSync(join(s, "main", "hank-cc1.jsonl")), "CC session 不受影响");
...
test("fix-codex-branch: dry-run 不动盘", () => {
  ...
  assert.ok(existsSync(join(s, "main", "tqt-rollout-1.jsonl")), "dry-run 不应移动");
